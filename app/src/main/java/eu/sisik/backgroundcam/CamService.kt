...
//    private lateinit var requestBuilder: CaptureRequest.Builder
    ...
    fun setupMediaRecorder() {
        ...
        val myDirectory = File(Environment.getExternalStorageDirectory(), "/DCIM/MomentRecorder/Videos/")
        ...
        val filePath =
            Environment.getExternalStorageDirectory()
                .toString() + "/DCIM/MomentRecorder/Videos/" + "Recorder" + formatter.format(now)  + ".mp4"
...
//                targetSurfaces.add(imageReader!!.surface)
                targetSurfaces.add(recordingSurface)
...
//                        requestBuilder.set(CaptureRequest.CONTROL_AF_MODE, CaptureResult.CONTROL_AF_MODE_CONTINUOUS_VIDEO)
                        try {
                            // Now we can start capturing
                            cameraCaptureSession.setRepeatingRequest(
                                requestBuilder.build(), captureCallback, null
                            )
                            mediaRecorder?.start()
